...
\begin{itemize}
    \item \textbf{Detección de Interferencia de Shell}: Se identificó que el \textit{Blue Team} configuró PowerShell como el \textit{DefaultShell} de OpenSSH. PowerShell puede inyectar caracteres de control (BOM) y encabezados en el \textit{stream} \texttt{stdio}, rompiendo el JSON-RPC.
...
\begin{itemize}
    \item \texttt{scp windows\_mcp.py user2@192.168.56.10:C:\textbackslash Python312\textbackslash mcp\_server.py} (Resincronización del servidor)
    \item \texttt{ssh user2@192.168.56.10 "reg add HKLM\textbackslash SOFTWARE\textbackslash OpenSSH /v DefaultShell /d C:\textbackslash Windows\textbackslash System32\textbackslash cmd.exe /f"} (Cambio de shell a CMD)
    \item \texttt{ssh user2@192.168.56.10 "powershell -ExecutionPolicy Bypass -File C:\textbackslash fix\_vm\_ssh.ps1"} (Ejecución de fix global: SSH=CMD, Port 8000 opened)
    \item \textbf{Confirmación}: Shell verificado como \texttt{c:\textbackslash windows\textbackslash system32\textbackslash cmd.exe}.
\end{itemize}

\subsection*{Verificación y Simulación Final (Exito)}
...
\begin{itemize}
    \item \textbf{Comando}: \texttt{echo '\{"jsonrpc": "2.0", "method": "initialize", ...\}' | ssh user2@192.168.56.10 "C:\textbackslash Python312\textbackslash python.exe C:\textbackslash Python312\textbackslash mcp\_server.py"}
...
\begin{itemize}
    \item Asegurar que la IP \texttt{192.168.56.10} está en \texttt{known\_hosts} (ejecutado \texttt{ssh-keyscan}).
    \item Reiniciar el servidor MCP en el IDE para limpiar sesiones SSH zombis.
    \item Verificar que el \textit{path} de Python en la configuración del servidor MCP en el Host coincide con \texttt{C:\textbackslash Python312\textbackslash python.exe}.
\end{itemize}

\textbf{FASE ALTERNATIVA: Túnel Directo por HTTP / SSE}
...
Se requiere sustituir el bloque \texttt{mcpServers} por el siguiente extracto, el cual utiliza rutas absolutas y fuerza el modo por lotes (\textit{batch}) para prevenir el error \texttt{-32000}:
\begin{verbatim}
{
  "mcpServers": {
    "windows-vm": {
...
Para llevar a cabo la investigación forense post-incidente, se orquestó una sesión agéntica especializada. La información, restricciones y reglas directivas que guiaron este análisis automatizado fueron extraídas y definidas en el documento \texttt{incidentanalysis.md}. Bajo este marco normativo, se aseguró el cumplimiento estricto de la recolección no destructiva y el enfoque en los eventos relevantes, lo que derivó en la generación del siguiente volcado de hallazgos.
...
1 (Mayor) & Conexiones de red activas & \texttt{Get-NetTCPConnection \textbar{} Where-Object \{\$\_.State -eq 'Established' -or \$\_.State -eq 'Listen'\}} \\ \hline
2 & Procesos en ejecución & \texttt{Get-Process \textbar{} Select-Object Id, ProcessName, Path, StartTime} \\ \hline
3 & Sesiones de usuario y logons & \texttt{Get-WinEvent -FilterHashtable @\{LogName='Security'; Id=4624\}} \\ \hline
4 & Logs de Sysmon & \texttt{Get-WinEvent -FilterHashtable @\{LogName='Microsoft-Windows-Sysmon/Operational'; Id=1,3,11,13,16\}} \\ \hline
...
Script \texttt{InternalAudit.ps1} y \texttt{sys\_data.dat} & Recolector de logs Sysmon del Blue Team \\ \hline
\texttt{takeown}/\texttt{icacls} sobre LOLBins & Hardening LOLBins (táctica de ``tierra quemada'') \\ \hline
...
Configuración de Sysmon \texttt{WinNetSvc.xml} & Reglas de detección defensivas \\ \hline
...
\subsection{Historial de Comandos del Red Team (\texttt{ConsoleHost\_history.txt})}
Se localizó el archivo de historial de PowerShell en el perfil del usuario \texttt{Administrador} (que corresponde al perfil original del administrador, renombrado a \texttt{user2} por el Blue Team).
...
\textbf{Comandos ejecutados por el Red Team:}
\begin{verbatim}
...
\begin{quote}
\textbf{PRECAUCIÓN:} \textbf{Contraseña expuesta:} La contraseña de la cuenta backdoor \texttt{yishiego} es \textbf{\texttt{EstuveAqui.1234}}, visible en texto plano en el historial de comandos.
\end{quote}
...
Login interactivo a las 14:01:14 y Logout a las 14:01:48 ($\sim$34 segundos de sesión). No hay comandos adicionales registrados, probablemente usada para verificar que la cuenta \texttt{yishiego} persistía tras el reinicio.
...
\begin{itemize}
    \item \textbf{Confidencialidad:} Reconocimiento de la estructura de la app web (\texttt{SecureCatalog}). Sin evidencia de exfiltración de datos.
    \item \textbf{Integridad:} Cuenta backdoor \texttt{yishiego} creada. Firewall desactivado (\texttt{netsh advfirewall set allprofiles state off}). BitLocker desactivado.

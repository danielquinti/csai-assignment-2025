...
El análisis forense del capítulo anterior confirmó el compromiso del servidor \texttt{WIN-VNQSUL89MUA} por el \textit{Red Team}. En respuesta, el \textit{Blue Team} ejecutó una remediación estructurada mediante un agente LLM en Cursor, conectado a la máquina virtual por el protocolo \textit{MCP}. Las diez medidas defensivas MS-01 a MS-10 cerraron los vectores identificados en la investigación.
...
El archivo \texttt{ConsoleHost\_}\allowbreak\texttt{history.txt} del perfil de \texttt{user2} conservó en texto plano los comandos ejecutados por el \textit{Red Team}, incluida la contraseña de la cuenta backdoor. PSReadLine persiste el historial interactivo sin filtrado de secretos, convirtiendo este artefacto en un vector de exposición de credenciales cuando no existe registro centralizado de sesiones de consola.
...
\small
\begin{tabular}{|c|c|>{\raggedright\arraybackslash}p{0.76\textwidth}|}
\hline
...
V-03 & MS-03 & Firewall activo; WDAC bloquea \texttt{netsh.exe} vía \texttt{BlueTeam\_}\allowbreak\texttt{Deny\_}\allowbreak\texttt{LOLBins} \\ \hline
...
V-06 & MS-06 & Sysmon con grupo \texttt{Hardening}\allowbreak\texttt{Consola}\allowbreak\texttt{Interactiva} en \texttt{WinNetSvc}\allowbreak\texttt{.xml} \\ \hline
...
V-09 & MS-09 & \texttt{SeShutdown}\allowbreak\texttt{Privilege} vacío; \texttt{Shutdown}\allowbreak\texttt{WithoutLogon=0} \\ \hline
...
\begin{figure}[htbp]
    ...
    \centering
    \texttt{[Cursor / Agente LLM (Host)]} $\longleftrightarrow$ \texttt{[MCP windows-vm]} $\longleftrightarrow$ \texttt{[SSH :22 $\rightarrow$}\allowbreak\texttt{ PowerShell (VM)]}
    \end{minipage}
...
Se implementa un modelo de separación de privilegios en el \textit{guest}: la sesión cotidiana en consola VirtualBox queda desacoplada de la identidad administrativa. La cuenta \texttt{user1} opera como miembro del grupo \texttt{Usuarios}, sin privilegios de administración local. Mediante \texttt{secedit}, el derecho \texttt{SeInteractive}\allowbreak\texttt{LogonRight} se restringe a \texttt{user1}, de modo que el subsistema LSA solo admite inicios de sesión interactivos (LogonType~2) para esa cuenta. En paralelo, \texttt{user2} se incluye en \texttt{SeDeny}\allowbreak\texttt{Interactive}\allowbreak\texttt{LogonRight}, impidiendo el logon en la consola gráfica. Deliberadamente \textbf{no} se modifica \texttt{SeDeny}\allowbreak\texttt{Network}\allowbreak\texttt{LogonRight} para \texttt{user2}, preservando el acceso SSH durante la fase de securización.

Windows Hello for Business queda habilitado en política (\texttt{Enabled=1}) con \texttt{Require}\allowbreak\texttt{Security}\allowbreak\texttt{Device=0}, preparando autenticación basada en TPM sin exigir dispositivo de seguridad en el primer acceso por contraseña. El estado vigente asigna la consola a \texttt{user1}; \texttt{user2} permanece en \texttt{Administradores} como cuenta de ruptura de cristal (\textit{break-glass}).
...
Se fijan de forma persistente tres valores en \texttt{HKLM\textbackslash}\allowbreak\texttt{SOFTWARE\textbackslash}\allowbreak\texttt{Microsoft\textbackslash}\allowbreak\texttt{Windows\textbackslash}\allowbreak\texttt{CurrentVersion\textbackslash}\allowbreak\texttt{Policies\textbackslash}\allowbreak\texttt{System}: \texttt{dontdisplay}\allowbreak\texttt{lastusername}, \texttt{DontDisplay}\allowbreak\texttt{LastUserName} y \texttt{HideFast}\allowbreak\texttt{UserSwitching}. Winlogon consulta estas claves al renderizar LogonUI; con ellas activas, el operador debe introducir explícitamente el nombre de usuario y se deshabilita el cambio rápido entre cuentas.
...
Los tres perfiles de Windows Defender Firewall permanecen habilitados con acción predeterminada de bloqueo entrante y saliente. Las claves de política en \texttt{HKLM\textbackslash}\allowbreak\texttt{SOFTWARE\textbackslash}\allowbreak\texttt{Policies\textbackslash}\allowbreak\texttt{Microsoft\textbackslash}\allowbreak\texttt{WindowsFirewall} refuerzan que el servicio MPSSVC no acepte desactivación persistente.

Durante la fase de gestión agéntica se desplegó \texttt{Enforce-Firewall.ps1} con la tarea \texttt{BlueTeam\_}\allowbreak\texttt{FirewallEnforce}, que cada cinco minutos ---como \texttt{NT AUTHORITY\textbackslash SYSTEM}--- restauraba el firewall y las reglas operativas (HTTP, HTTPS, SSH y \textit{MCP}) salvo que existiera el marcador \texttt{.modo\_}\allowbreak\texttt{entrega}. Esta tarea se desregistra al ejecutar \texttt{Cierre-Operativo.ps1} (véase la subsección~\ref{subsec:cierre_operativo}).

La política WDAC \texttt{BlueTeam\_}\allowbreak\texttt{Deny\_}\allowbreak\texttt{LOLBins} impide la ejecución de \texttt{netsh.exe}. Se generó fusionando la plantilla \texttt{AllowAll.xml} con reglas \textit{Deny} a nivel \textit{FilePublisher} mediante \texttt{Merge-CIPolicy}; el XML resultante se convirtió a binario \texttt{.cip} con \texttt{ConvertFrom-CIPolicy} y se desplegó en modo \textit{enforce} con \texttt{CiTool.exe} tras reinicio. En modo entrega, el firewall conserva reglas entrantes limitadas a TCP~80 y TCP~443; la política WDAC sobre \texttt{netsh.exe} permanece vigente.
...
El volumen \texttt{C:} permanece en \texttt{Protection}\allowbreak\texttt{Status=On}, \texttt{Volume}\allowbreak\texttt{Status=FullyEncrypted}, con protectores TPM y de recuperación. Las directivas FVE en \texttt{HKLM\textbackslash}\allowbreak\texttt{SOFTWARE\textbackslash}\allowbreak\texttt{Policies\textbackslash}\allowbreak\texttt{Microsoft\textbackslash}\allowbreak\texttt{FVE} establecen \texttt{EnableBDE}\allowbreak\texttt{WithNoTPM=0}, \texttt{UseTPMPIN=0} (sin PIN de arranque, coherente con la portabilidad de la VM entre hosts VirtualBox) y \texttt{RDVDeny}\allowbreak\texttt{WriteAccess=1}. La política WDAC \texttt{BlueTeam\_}\allowbreak\texttt{Deny\_}\allowbreak\texttt{LOLBins} bloquea \texttt{manage-bde.exe}.
...
El esquema XML de Sysmon (\texttt{WinNetSvc}\allowbreak\texttt{.xml}) incorpora el grupo \texttt{Hardening}\allowbreak\texttt{Consola}\allowbreak\texttt{Interactiva}, recargado en caliente mediante \texttt{WinNetSvc.exe -c}. Las reglas cubren shells interactivas (\texttt{cmd.exe}, \allowbreak \texttt{powershell.exe}, \allowbreak \texttt{pwsh.exe}), utilidades empleadas en el ataque (\texttt{netsh.exe}, \allowbreak \texttt{manage-bde.exe}, \allowbreak \texttt{net.exe}, \allowbreak \texttt{net1.exe}, \allowbreak \texttt{shutdown.exe}) y patrones de línea de comandos asociados a gestión de cuentas y desactivación del firewall. Cualquier sesión LogonType~2 que invoque dichas herramientas genera un evento ID~1 auditable.
...
El perfil global de PowerShell (\texttt{profile.ps1}) configura \texttt{Set-PSReadLineOption}\allowbreak\texttt{ -HistorySaveStyle SaveNothing}, impidiendo que nuevas sesiones escriban historial en disco; los ficheros \texttt{ConsoleHost\_}\allowbreak\texttt{history.txt} preexistentes fueron eliminados. Se habilitan la transcripción de PowerShell y el Script Block Logging mediante directivas en \texttt{HKLM\textbackslash}\allowbreak\texttt{SOFTWARE\textbackslash}\allowbreak\texttt{Policies\textbackslash}\allowbreak\texttt{Microsoft\textbackslash}\allowbreak\texttt{Windows\textbackslash}\allowbreak\texttt{PowerShell}, con salida centralizada en \texttt{C:\textbackslash}\allowbreak\texttt{Windows\textbackslash}\allowbreak\texttt{System32\textbackslash}\allowbreak\texttt{Config\textbackslash}\allowbreak\texttt{TxR\textbackslash}\allowbreak\texttt{PSTranscripts} y ACL restrictiva sobre ese directorio.
...
La política de cuentas locales establece umbral de bloqueo de \textbf{tres} intentos fallidos, duración de bloqueo de treinta minutos y ventana de observación de quince minutos. La longitud mínima es de dieciséis caracteres, con complejidad habilitada (\texttt{Password}\allowbreak\texttt{Complexity=1}), caducidad máxima de noventa días e historial de veinticuatro contraseñas únicas.
...
Mediante \texttt{secedit}, el privilegio \texttt{SeShutdown}\allowbreak\texttt{Privilege} queda vacío en la política de derechos de usuario: ninguna cuenta local conserva por defecto el derecho a apagar o reiniciar el equipo desde una sesión interactiva. \texttt{Shutdown}\allowbreak\texttt{WithoutLogon=0} impide apagar la máquina desde LogonUI sin autenticación previa. \texttt{shutdown.exe} figura además en las reglas Sysmon de MS-06.
...
Tras completar MS-01 a MS-10 y la verificación automatizada de controles, se ejecutó \texttt{Cierre-Operativo.ps1} como paso único de pechado. El script, ubicado en \texttt{C:\textbackslash}\allowbreak\texttt{Windows\textbackslash}\allowbreak\texttt{System32\textbackslash}\allowbreak\texttt{Drivers\textbackslash}\allowbreak\texttt{en-US\textbackslash}\allowbreak\texttt{NetworkData\textbackslash}\allowbreak\texttt{Cierre-Operativo.ps1}, consolida las acciones que en el despliegue original residían en \texttt{nuke.ps1}: cerrar el perímetro de gestión remota y dejar la VM en postura de entrega, sin revertir las defensas reactivas desplegadas en este capítulo.

El script ejecuta en secuencia: creación del marcador \texttt{.modo\_}\allowbreak\texttt{entrega} (señal para que \texttt{Enforce-Firewall.ps1} deje de actuar); desregistro de la tarea \texttt{BlueTeam\_}\allowbreak\texttt{FirewallEnforce}; reconfiguración del firewall con reglas entrantes únicamente en TCP~80 y TCP~443; detención de procesos Python del servidor \textit{MCP}; detención y desinstalación de OpenSSH con eliminación de claves autorizadas; y limpieza de historial de PowerShell y archivos en \texttt{\%TEMP\%}.

Las medidas defensivas MS-01 a MS-10 permanecen activas tras el cierre: BitLocker, WDAC (\texttt{BlueTeam\_}\allowbreak\texttt{Deny\_}\allowbreak\texttt{LOLBins}), Sysmon, \texttt{AdminGuard}, políticas de identidad, lockout y ACLs de \textit{SecureCatalog} no se revierten. Lo que cambia es la \textbf{disponibilidad de gestión remota}: la VM queda administrable exclusivamente por consola VirtualBox (con \texttt{user1} como cuenta operativa) y expuesta únicamente al tráfico HTTP/HTTPS de la aplicación.
...
\small
\begin{tabular}{|>{\raggedright\arraybackslash}p{0.13\textwidth}|>{\raggedright\arraybackslash}p{0.49\textwidth}|>{\raggedright\arraybackslash}p{0.13\textwidth}|>{\raggedright\arraybackslash}p{0.11\textwidth}|}
\hline
...
\textbf{CECI} \newline (\textit{Cobertura de Eventos de Consola Interactiva}) & Proporción de comandos ejecutados en sesiones LogonType~2 que generan evento Sysmon ID~1 conforme al grupo \texttt{Hardening}\allowbreak\texttt{Consola}\allowbreak\texttt{Interactiva}. & Mensual & $100\,\%$ \\ \hline
